...
const buildStoresAppJs = () => {
  const lines = [
    ...
    "    const prefecture = str(row[requiredKeys.prefectureKey]);",
    ...
    "    const badgesHtml = visibleBadges.map((badge) => '<span style=\"display:inline-flex;align-items:center;gap:4px;border-radius:999px;background-color:' + badge.color + ';border:1px solid ' + badge.color + ';padding:3px 8px;font-size:11px;line-height:1.3;color:#000;font-weight:600;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;\">' + badge.text + '</span>').join('');",
    "    const extraBadgeHtml = extraBadgeCount > 0 ? '<span style=\"display:inline-flex;align-items:center;border-radius:999px;border:1px solid #fed7aa;background:#fff7ed;padding:3px 8px;font-size:11px;line-height:1.3;color:#c2410c;font-weight:600;\">+' + extraBadgeCount + '</span>' : '';",
    "    return '<article class=\"target-stores__card\" style=\"background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;box-shadow:0 1px 0 rgba(15,23,42,0.04);padding:14px;\"><div style=\"margin-bottom:8px;display:flex;flex-wrap:wrap;gap:6px;\">' + badgesHtml + extraBadgeHtml + '</div><h3 class=\"target-stores__name\" style=\"margin:0 0 6px;background:#ffffff;color:#1d4ed8;font-size:14px;line-height:1.4;font-weight:700;\">' + storeName + '</h3><p class=\"target-stores__address\" style=\"margin:0;color:#64748b;font-size:12px;line-height:1.6;\">' + (postal ? ('〒' + postal + ' ') : '') + prefecture + address + '</p></article>';",
    "  };",

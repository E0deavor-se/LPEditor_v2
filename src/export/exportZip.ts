...
const buildStoresAppJs = () => {
  const lines = [
    ...
    "    const cardElements = cardsNode ? Array.from(cardsNode.querySelectorAll('[data-store-id]')) : [];",
    "    const cardDisplayModes = cardElements.map((card) => { const display = card.style.display; return display && display !== 'none' ? display : 'block'; });",
    ...
    "          if (!name.includes(normalizedKeyword) && !address.includes(normalizedKeyword) && !postal.includes(normalizedKeyword)) { return; }",
    "        }",
    "        if (selectedPrefecture) {",
    "          const prefecture = str(row[requiredKeys.prefectureKey]).trim();",
    "          if (prefecture !== selectedPrefecture) { return; }",
    "        }",
    ...
    "              if (!isTruthy(str(row[key]).trim())) { return; }",
    "            }",
    "          }",
    "        }",
    ...
    "    const render = () => {",
    "      updateFilterButtons();",
    "      const filteredIndexes = filterRowIndexes();",
    ...
    "      const visibleSet = new Set(pageIndexes);",
    "      if (pageNode) { pageNode.textContent = String(page) + '/' + String(totalPages); }",
    "      const countLabel = countNode ? (countNode.getAttribute('data-store-count-label') || '表示件数') : '表示件数';",
    "      if (countNode) { countNode.textContent = countLabel + ': ' + filteredIndexes.length + '件'; }",
    "      if (prevButton) { prevButton.disabled = page <= 1; }",
    "      if (nextButton) { nextButton.disabled = page >= totalPages; }",
    "      const empty = filteredIndexes.length === 0;",
    "      if (emptyNode) { emptyNode.style.display = empty ? 'block' : 'none'; }",
    "      if (cardsNode) {",
    "        cardsNode.style.display = empty ? 'none' : 'grid';",
    "        cardElements.forEach((card, index) => { card.style.display = visibleSet.has(index) ? (cardDisplayModes[index] || 'block') : 'none'; });",
    "      }",
    "    };",
    ...
    "    if (nextButton) { nextButton.addEventListener('click', () => { const filteredIndexes = filterRowIndexes(); const totalPages = Math.max(1, Math.ceil(filteredIndexes.length / configuredPageSize)); page = Math.min(totalPages, page + 1); render(); }); }",
